...
- Network reachability to your **Dify** instance. If Dify is only reachable over a
  private network (e.g. Tailscale/NetBird/WireGuard), the host must be on that network.
...
- **Networking**: compose uses host networking so the backend can resolve Dify
  over a private network (e.g. Tailscale/NetBird/WireGuard). This needs Linux (it won't
